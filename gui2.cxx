...
HelloWorld::HelloWorld() : m_adjustment_amp(0.0, 0.0, 1000.0, 0.0000001, 0.0001, 0.0), // for the spinbutton
	...
	m_table(5, 2, true),
	m_button1("PT1000 resistance @ 10uA"),
...
void HelloWorld::on_button1_clicked(Glib::ustring data)
{
	...
	cout << "Also, try to measure voltage at 10 uA" << endl;
...
void HelloWorld::on_button_ivcurve_clicked(Glib::ustring data)
{
	...
	strftime(tmptime,10,"%X",timeinfo);
	for(int i=0;i<100;i++) {
		double j = -1.3e-5+1e-9*pow(10,0.05*i); // finish off making it go from 1e-4 to 1e-2
